...
'取得产品在数据库中的ID
dim proId
sql="select top 1 * from product order by proid desc"
set rs = conn.execute(sql)
proId = rs("proid")
'关联图片
sql="update productpic set productid = "&cstr(proId)&" where productid = 0 "
conn.execute(sql)
rs.close
conn.close
%>
...
<body>
<script>alert("添加成功,请继续添加.");window.history.go(-1);</script>
</body>
</html>
