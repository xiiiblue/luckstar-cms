...
if action = "del" and proid <> "" then
	sql = "delete from product where proid in ("&proid&")"
	'response.Write(sql)
...
if action = "move" and proid <> "" and proclassid<>0 then
	sql = "update product set proclassid = "&proclassid&" where proid in ("&proid&")"
	'response.Write(sql)
	Set rs= Server.CreateObject("ADODB.Recordset")
		rs.open sql,conn,1,3
	set rs=nothing
...
if action = "order" and proorderid <> "" and proorder <> "" then
	...
	for i = 0 to ubound(proorderids)
		sql = "update product set proorder = "&proorders(i)&" where proid = "&proorderids(i)
		conn.execute(sql)
	next
end if
