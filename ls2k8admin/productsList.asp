...
<head>
	...
	<script src="js/productsList.js" type="text/javascript"></script><!--本页专有JS-->
</head>
<body>
<!--header开始-->
...
<table width="97%" align="center" border="0" cellspacing="0" cellpadding="0">
  ...
  <tr>
	...
	<a href="productsAdd.asp?proClass=<%=request("proClass")%>" class="coolBtn2">添加产品</a>	
	</td>
  </tr>
  <tr>
...
outstr = outstr&"<option value=""0"">请选择分类</option>"&vbcrlf
...
%>
      ...
      <tr>
		...
		<!--列表开始-->
		<form method="post" name="formProList" id="formProList" action="productsAlter.asp">
		...
		<table width="100%" border="0" cellpadding="2" cellspacing="2" class="title">
			<tr align="center" bgcolor="#F6F6F6">
				...
				<td width="5%">NO</td>
				<td width="25%" bgcolor="#F6F6F6">产品名称</td>
				<td width="5%">图片</td>
				<td width="10%">时间</td>
				<td width="25%">状态</td>
...
function showProList(rs,iPageSize)
	...
	if not (rs.eof and rs.bof) then
		...
		rs.AbsolutePage=curPage	'初始化当前页数
		dim i:i=0
		do while not rs.eof
			i = i + 1
%>
		<table width="100%" border="0" cellpadding="2" cellspacing="2" class="proList">
          <tr align="center">
            ...
            <td width="25%" align="left">
				  <a href="productsEdit.asp?action=edit&proid=<%=rs("proid")%>" title="<%=rs("proname_cn")%>">
				  <%if xiii_language <> "en" then%>
			...
			<a href="productsStatus.asp?proid=<%=rs("proid")%>&toggle=show">显示</a>
			<%if rs("proshow") = 1 then%>
				<span class="red">(√)</span>
			<%else%>
				<span class="green">(×)</span>
			<%end if%>
			<a href="productsStatus.asp?proid=<%=rs("proid")%>&toggle=up">固顶</a>
			<%if rs("proup") = 1 then%>
				<span class="red">(√)</span>
			<%else%>
				<span class="green">(×)</span>
			<%end if%>
			<a href="productsStatus.asp?proid=<%=rs("proid")%>&toggle=commend">推荐</a>
			<%if rs("procommend") = 1 then%>
				<span class="red">(√)</span>
			<%else%>
				<span class="green">(×)</span>
			<%end if%>
			<a href="productsStatus.asp?proid=<%=rs("proid")%>&toggle=hot">热点</a>
			<%if rs("prohot") = 1 then%>
				<span class="red">(√)</span>
			<%else%>
				<span class="green">(×)</span>
...
<!--			<input type="button" value="变更" class="altOrderSubmit" />-->			</td>
            <td width="5%">
			<a href="productsEdit.asp?action=edit&proid=<%=rs("proid")%>">
			<img src="images/edit.gif" width="22" height="22" />			</a>			</td>
            <td width="5%">
			<a href="productsAlter.asp?action=del&proid=<%=rs("proid")%>" onclick="return confirm('确定删除吗?')">
...
if(action="search") then
	...
	if proClass<>0 then
		dim proClasses:proClasses = getSubClasses("productclass",proClass)
