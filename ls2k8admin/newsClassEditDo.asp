...
if classId <> 0 then
	'更新中英文名称
	sql = "update newsclass set classname_cn = '"&classNameCn&"' ,classname_en = '"&classNameEn&"' where id ="&classId
	conn.execute(sql)
	...
	elseif left(selfCode,selfLevel*4-4) = parentCode then
		...
		sql = "update newsclass set classcode='"&leftCode&"'+mid(classcode,"&(selfLevel*4+1)&",len(classcode)) where classcode like'"&selfCode&"%'"
		'response.write sql
		conn.execute(sql)
		'response.write("成功")
		errMsg = "更改成功"
